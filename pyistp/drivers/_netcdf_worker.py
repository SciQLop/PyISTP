"""Runs inside a dedicated subprocess.

netCDF4/HDF5 is not thread-safe — not even under a global mutex (see
github.com/Unidata/netcdf-c/issues/2496, which crashes with the same
H5T__vlen_disk_isnull / H5VL_blob_specific signature we hit locally). A
Python-level lock cannot fix a bug in the native library's own global
state, so all netCDF4 calls are confined to this single subprocess,
reached from the driver over a pipe. Handles are keyed by uuid4 (not a
counter) so a stale handle from a crashed-and-respawned worker reliably
raises KeyError instead of silently addressing an unrelated dataset.
"""

import os
import uuid

# Numpy's BLAS backend lazily spawns its own native worker thread pool
# on first use. That's exactly the "another thread merely exists in the
# process" trigger observed to corrupt HDF5's internal state — pin it to
# a single thread before numpy is imported so this process genuinely
# never has more than one thread touching native code.
...
class _RealDriver:
    """The actual netCDF4 driver logic. Runs single-threaded inside the
    worker process, so it needs no synchronization of its own."""
    ...
    while True:
        ...
        try:
            if op == 'open':
                _, file = request
                handle = uuid.uuid4()
                drivers[handle] = _RealDriver(file)
                conn.send(('ok', handle))
            elif op == 'close':
                _, handle = request
                driver = drivers.pop(handle, None)
                if driver is not None:
                    driver.close()
                conn.send(('ok', None))
            elif op == 'call':
                _, handle, method, args, kwargs = request
                result = getattr(drivers[handle], method)(*args, **kwargs)
                conn.send(('ok', result))
            else:
